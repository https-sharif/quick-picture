...
const Form : React.FC<Props> = ({ prompt, onInputChange, loading, handleClick, handleKeyPress, error, queryType, setQueryType }) => {
    const [randomizedIndex, setRandomizedIndex] = useState<number>(Math.floor(Math.random() * 10));

    const placeholdersValue : string[] =   ['Cats', 'Foods', 'Birds', 'Flowers', 'Mountains', 'Beaches', 'Sunsets', 'Sunrises', 'Stars', 'Galaxies'];
        ...
        <>
            <div className="mb-16">
                <form className="flex items-center flex-col justify-center relative">
                    <textarea 
                        id="prompt" 
                        placeholder={placeholdersValue[randomizedIndex]}
                        disabled={loading}
                        value={prompt}
                        rows={2}
                        onChange={(e) => {
