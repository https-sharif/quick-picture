...
const User: React.FC<UserProps> = ({
  ...
  return (
    <div className="flex items-center justify-around min-w-[80%] mt-16">
      <div className="flex justify-around w-3/4">
        <img src={profile_image.large} className="object-contain w-60 mr-5" alt={`Profile of ${name}`} />
        <div className="w-3/5 relative">
          <p className="text-4xl font-bold">{name}</p>
          <p className="text-gray-600 text-xl">@{username}</p>
          <p className="text-lg mt-5 mb-5">{bio}</p>
          <div className="w-1/4 h-16 flex items-center absolute bottom-5">
            {instagram_username && (
              <a className="pr-2"
